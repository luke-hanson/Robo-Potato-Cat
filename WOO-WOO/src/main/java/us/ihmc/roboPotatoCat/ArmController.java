...
public class ArmController implements RobotController
{
    ...
    */
    ...
    private int i = 0; //first counter
    private int j = 0;// second counter
    ...
    public void secondController()
    {
        ...
        if (i > 1000)
        {
            ...
            positionError = desiredPositionRadians.getDoubleValue() - (robot.getSecondAngularPosition()+1);
            i--;
        }
        if (i < 1000)
        {
            positionError = desiredPositionRadians.getDoubleValue() - (robot.getSecondAngularPosition()+5);
            i++;
        }


        // ERROR term: Compute the difference between the desired position the pendulum and its current position
        //positionError = desiredPositionRadians.getDoubleValue() - (robot.getSecondAngularPosition());
        //secondForther();
    ...
    public void secondForther()
    {
        if (i > 100)
        {
            desiredPositionRadians.setValueFromDouble(desiredPositionRadians.getDoubleValue() * 2);
        }
        else
        {
            desiredPositionRadians.setValueFromDouble(Math.PI);
            i = 0;
        }
